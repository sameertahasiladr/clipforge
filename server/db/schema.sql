-- ====================================================================
-- ClipForge AI — PostgreSQL Database Schema
-- Production-Ready Relational Database Models
-- ====================================================================
...
CREATE TABLE IF NOT EXISTS users (
    ...
    avatar_url TEXT,
    role VARCHAR(50) DEFAULT 'creator', -- creator, pro, agency, admin
    plan_tier VARCHAR(50) DEFAULT 'free', -- free, creator, pro, agency
...
-- 2. Projects Table (One source video project)
CREATE TABLE IF NOT EXISTS projects (
    ...
    status VARCHAR(50) DEFAULT 'completed', -- queued, processing, completed, failed
...
CREATE INDEX IF NOT EXISTS idx_source_videos_project ON source_videos(project_id);

-- 4. Clips Table (Generated 10-15s vertical clips)
CREATE TABLE IF NOT EXISTS clips (
    ...
    thumbnail_url TEXT,
    video_url TEXT,
    status VARCHAR(50) DEFAULT 'draft', -- draft, queued, scheduled, published
    caption_style VARCHAR(50) DEFAULT 'dynamic', -- minimal, bold, dynamic, highlight
    font_family VARCHAR(50) DEFAULT 'Plus Jakarta Sans',
    caption_position VARCHAR(20) DEFAULT 'bottom', -- top, middle, bottom
...
-- 5. Clip Captions (Word-level timings & styling)
CREATE TABLE IF NOT EXISTS clip_captions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    clip_id UUID NOT NULL REFERENCES clips(id) ON DELETE CASCADE,
    word_timings JSONB NOT NULL, -- [{word: "Nobody", start: 0.1, end: 0.4, highlight: true}]
    full_text TEXT NOT NULL,
    style_config JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_clip_captions_clip_id ON clip_captions(clip_id);

-- 6. Social Accounts (OAuth connections)
...
CREATE TABLE IF NOT EXISTS publishing_jobs (
    ...
    platform VARCHAR(50) NOT NULL, -- instagram, facebook, youtube
    status VARCHAR(50) DEFAULT 'queued', -- queued, uploading, processing, published, failed
    publish_mode VARCHAR(20) DEFAULT 'immediate', -- immediate, scheduled
...
-- 8. Scheduled Posts (For Calendar & Scheduler view)
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    plan_name VARCHAR(50) NOT NULL, -- free, creator, pro, agency
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    type VARCHAR(50) DEFAULT 'info', -- info, success, warning, publish, ai
